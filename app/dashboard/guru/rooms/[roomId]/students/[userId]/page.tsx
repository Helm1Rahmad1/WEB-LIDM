...
// Component untuk menampilkan huruf hijaiyah dengan status
function LetterProgressDisplay({
  letter,
  isCompleted
}: {
  letter: { arabic_char: string; latin_name: string; hijaiyah_id: number }
  isCompleted: boolean
}) {
  return (
    <motion.div
      className={`relative p-3 rounded-xl flex flex-col items-center justify-center transition-all ${isCompleted
        ? 'bg-gradient-to-br from-green-500 to-green-600 text-white shadow-lg'
        : 'bg-gray-100 text-gray-400 shadow-sm'
        }`}
      whileHover={{ scale: 1.05, y: -2 }}
      title={`${letter.latin_name} - ${isCompleted ? 'Selesai ✅' : 'Belum Selesai'}`}
    >
      {isCompleted && (
        <motion.div
      ...
      <div className="text-2xl font-bold mb-1">{letter.arabic_char}</div>
      <div className={`text-xs font-medium ${isCompleted ? 'text-white/90' : 'text-gray-500'
        }`}>
        {letter.latin_name}
      </div>
    </motion.div>
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch all hijaiyah letters
        const hijaiyahRes = await apiClient.get('/api/hijaiyah')
        const allHijaiyahLetters = hijaiyahRes.data.letters || []
        console.log('✅ All Hijaiyah Letters:', allHijaiyahLetters)
        setAllLetters(allHijaiyahLetters)
        ...
        const letterProgressData = letterProgressRes.data.progress || []
        setLetterProgress(letterProgressData)
        ...
        <Card className="border-0 bg-white/95 shadow-2xl rounded-3xl">
          <CardHeader className="bg-gradient-to-r from-purple-50 to-pink-50 p-8">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <motion.div
                  className="p-3 rounded-2xl bg-gradient-to-br from-purple-600 to-purple-700 shadow-lg"
                  whileHover={{ rotate: 360 }}
                >
                  <Star className="h-8 w-8 text-white" />
                </motion.div>
                <div>
                  <CardTitle className="text-3xl font-bold">Progress Huruf Hijaiyah</CardTitle>
                  <CardDescription className="text-lg">Huruf yang telah dikuasai</CardDescription>
                </div>
              </div>
              <div className="text-right">
                <div className="text-4xl font-black text-purple-600">
                  {letterProgress.length}/{allLetters.length}
                </div>
                <div className="text-sm text-gray-600 font-medium">Huruf Selesai</div>
              </div>
            </div>
          </CardHeader>
          <CardContent className="p-8">
            {allLetters.length > 0 ? (
              <motion.div
                ...
                animate="visible"
              >
                {allLetters.map((letter: any) => {
                  const isCompleted = letterProgress.some(
                    (lp: LetterProgress) => lp.hijaiyah_id === letter.hijaiyah_id
                  )
                  return (
                    <motion.div key={letter.hijaiyah_id} variants={fadeInUp}>
                      <LetterProgressDisplay
                        letter={letter}
              ...
              <div className="text-center py-8 text-gray-500">
                <p>Belum ada data huruf hijaiyah</p>
              </div>
            )}
          </CardContent>
        </Card>
